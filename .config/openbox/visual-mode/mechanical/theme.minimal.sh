...
# ~/.config/openbox/visual-mode/mechanical/theme.minimal.sh: Setup minimal mechanical theme.
...
partially()
{
    ...
    sed -e "/menu.items.active.text.color/s|:.*|: ${MECH_MIN_OB_MENU_ITM}|" \
        -e "/menu.title.text.color/s|:.*|: ${MECH_MIN_OB_MENU_TTL}|"        \
    ...
    sed -e "/foreground/s|:.*;|:       ${MECH_MIN_ROFI_FRGRND};|" \
        -e "/accent/s|:.*;|:           ${MECH_MIN_ROFI_ACCENT};|" \
