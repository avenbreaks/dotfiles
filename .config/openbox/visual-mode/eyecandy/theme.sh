...
# ~/.config/openbox/visual-mode/eyecandy/theme.sh: Setup non-minimal eyecandy theme.
...
partially()
{
    ...
    sed -e "/menu.items.active.text.color/s|:.*|: ${EYEC_OB_MENU_ITM}|" \
        -e "/menu.title.text.color/s|:.*|: ${EYEC_OB_MENU_TTL}|"        \
        -i "${EYEC_BUTTON_DIR}/themerc"
    # Set rofi accent color from "~/.joyful_desktop".
    sed -i "/accent/s|:.*;|:           ${EYEC_ROFI_ACCENT};|" "${ROFI_COLORSCHEMES_DIR}/fullscreen.rasi"
    sed -e "/foreground/s|:.*;|:       ${EYEC_ROFI_FRGRND};|" \
        -e "/accent/s|:.*;|:           ${EYEC_ROFI_ACCENT};|" \
