...
# ~/.config/openbox/visual-mode/eyecandy/theme.minimal.sh: Setup minimal eyecandy theme.
...
partially()
{
    ...
    sed -e "/menu.items.active.text.color/s|:.*|: ${EYEC_MIN_OB_MENU_ITM}|" \
        -e "/menu.title.text.color/s|:.*|: ${EYEC_MIN_OB_MENU_TTL}|"        \
        -i "${EYEC_BUTTON_DIR}/themerc"
    ...
    sed -e "/foreground/s|:.*;|:       ${EYEC_MIN_ROFI_FRGRND};|" \
        -e "/accent/s|:.*;|:           ${EYEC_MIN_ROFI_ACCENT};|" \
        -i "${ROFI_COLORSCHEMES_DIR}/sidebar.rasi"
}
...
apply_all()
{
    ...
    # Set Openbox margin, then reconfigure Openbox.
    "${VISMOD_DIR}/margin-dunst-rofi.sh" minimal
    openbox --reconfigure
