...

{
    ...
    if [ -n "$ALBUM_COVER" ]; then
        ...
        if [ -f "$COVER_PNG" ]; then
            notify-send.sh -r 78 -i "$COVER_PNG" '' "<span size='small'>$(mpc -p "$MPD_PORT" --format '%artist%' current)</span>\n$(mpc -p "$MPD_PORT" --format '%title%' current)"
        fi
    else
        notify-send.sh -r 78 -i "$MUSIC_ICON" '' "<span size='small'>$(mpc -p "$MPD_PORT" --format '%artist%' current)</span>\n$(mpc -p "$MPD_PORT" --format '%title%' current)"
    fi
