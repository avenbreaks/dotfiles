...
# ~/.config/ncmpcpp/scripts/album-art.sh: Show album-art in ncmpcpp.
    ...
    if [ -x "$(command -v xprop)" ] && [ -x "$(command -v xwininfo)" ]; then
        ...
        while :; do
            ...
            if [ -n "$GET_W" -o -n "$GET_H" ]; then
                W_RES="$(echo "${GET_W}/3.521" | bc)"
                #H_RES="$(echo "${GET_H}/1.52" | bc)"
            fi
            sleep .25s
...
} & # DEBUG: Just remove "&" symbol on left side then execute with trace paramater, `yash -x ./album-art.sh`.
